...
<template>
  ...
  >
    ...
    <!-- ═══ MODO CONFIRMAÇÃO ═══ -->
    <div
      v-if="modoConfirmacao"
      class="space-y-6 pt-2"
    >
      <div
        v-if="faturaAtivaVisualizada"
      ...
      >
        <div class="bg-parchment p-4 rounded-2xl border border-stone shadow-subtle">
          ...
          <p class="text-2xl font-display text-charcoal break-words">
            {{ formatarBRL(formatarDinheiro(totalPeriodoSelecionado)) }}
          </p>
          <p class="text-[10px] text-graphite font-bold mt-1 uppercase opacity-60">
            {{ totalLancamentosPeriodoSelecionado }} lançamentos
          </p>
        </div>
        ...
        <div class="bg-parchment p-4 rounded-2xl border border-stone shadow-subtle">
          ...
          <p class="text-2xl font-display text-ember">
            {{ nettingTransferencias.length }} Acertos
          </p>
          <p class="text-[10px] text-graphite font-bold mt-1 uppercase opacity-60">
            serão cobrados
          </p>
      ...
      <div
        v-if="faturaAtivaVisualizada && temContaFixaPendente"
        class="p-4 rounded-2xl bg-ember/5 border border-ember/20 flex gap-3 items-start animate-in fade-in slide-in-from-top-1"
      >
        ...
        <div>
          <p class="text-xs font-bold text-ember uppercase tracking-tight">
            Contas fixas pendentes!
          </p>
          <p class="text-[11px] text-graphite font-semibold mt-0.5 leading-tight">
            Ainda existem contas fixas deste mês que não foram lançadas. Deseja fechar mesmo assim?
          </p>
        </div>
      </div>
    </div>
    ...
    >
      ...
      <div class="space-y-3">
        <h4 class="text-[10px] font-bold uppercase tracking-widest text-graphite ml-1">
          Gerenciar Período Aberto
        </h4>
        <div
          class="relative"
          @focusout="isDropdownAbertosOpen = false"
        >
          <div 
          ...
          >
            <span class="flex items-center gap-2.5">
              <Lock
                ...
                class="w-3.5 h-3.5 text-graphite opacity-50 shrink-0"
              />
              <span
                v-else
                class="w-2.5 h-2.5 rounded-full bg-meadow animate-pulse shrink-0"
              />
            ...
            <ChevronDown
              class="w-4 h-4 text-graphite pointer-events-none transition-transform duration-300"
          ...
          <transition name="dropdown-slide">
            <div
              ...
              class="absolute left-0 w-full mt-1.5 max-h-48 overflow-y-auto bg-card border border-stone rounded-xl shadow-xl z-50 py-2 custom-scrollbar"
            >
              <div 
      ...
      <div class="space-y-3">
        ...
        <div class="grid gap-2">
          ...
          <div
            v-if="mesesTrancadosOpcoes.length === 0"
            class="text-center py-4 border border-dashed border-stone rounded-2xl"
          >
            <p class="text-[11px] text-graphite font-semibold opacity-60 italic">
      ...
      <div class="bg-parchment p-5 rounded-2xl border border-stone shadow-subtle">
        <div class="flex items-start gap-4">
          <div
            v-if="faturaSelecionadaFechada"
            class="w-10 h-10 rounded-xl bg-meadow/10 flex items-center justify-center shrink-0 border border-meadow/20"
          >
            <CheckCircle2
              class="w-5 h-5 text-meadow"
              stroke-width="3"
            />
          </div>
          <div
            v-else
            class="w-10 h-10 rounded-xl bg-ember/10 flex items-center justify-center shrink-0 border border-ember/20"
          >
            <CheckCircle2
              class="w-5 h-5 text-ember opacity-30"
              stroke-width="3"
            />
          </div>
    ...
    <template #footer>
      ...
      >
        <Button
          ...
          @click="handleCancelarConfirmacao"
        >
          Cancelar
        </Button>
        <Button
          ...
          @click="handleConfirmarEncerramento"
        >
          Arquivar Mês
        </Button>
      </div>
      ...
      >
        ...
        <Button
          v-else
          variant="primary"
          class="!bg-midnight hover:!bg-charcoal text-white font-bold uppercase tracking-widest text-[10px] h-12 shadow-md"
          :disabled="isFecharPeriodoBloqueado"
          @click="handleEncerrar"
        >
          Encerrar Mês
        </Button>
      </div>
